...
export default class Session extends React.Component
{
	...
	render()
	{
		...
		if (props.session.isInProgress() && !state.ringing)
			noRemoteVideo = <div className='message'>đang kết nối ...</div>;
		else if (state.ringing)
			noRemoteVideo = <div className='message'>đang đổ chuông ...</div>;
		else if (state.localHold && state.remoteHold)
			noRemoteVideo = <div className='message'>cả hai đang giữ máy</div>;
		else if (state.localHold)
			noRemoteVideo = <div className='message'>bạn đang giữ máy</div>;
		else if (state.remoteHold)
			noRemoteVideo = <div className='message'>khách đang giữ máy</div>;
		else if (!state.remoteHasVideo)
			noRemoteVideo = <div className='message'>khách không có video</div>;
		...
		session.on('accepted', (data) =>
		{
			...
			if (session.direction === 'outgoing')
			{
				this.props.onNotify(
					{
						level : 'success',
						title : 'Cuộc gọi đã được trả lời'
					});
			}
